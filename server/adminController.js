...
function loginAdmin(req, res) {
    try {
        ...
        if (!bcrypt.compareSync(String(password), admin.password)) {
            return res.status(401).json({
                success: false,
                message: "Invalid username or password."
            });
...
function updateAdminSettings(req, res) {
    try {
        ...
        if (!username || !currentPassword || !displayName) {
            return res.status(400).json({
                success: false,
                message: "Username, current password and display name are required."
            });
        }
        ...
        if (newPassword) {
            ...
            passwordHash = bcrypt.hashSync(String(newPassword), 12);
        }
...
async function forgotAdminPassword(req, res) {
    try {
        ...
        const admin = db.prepare(`
            SELECT id, username, admin_email
            FROM admin_settings
            WHERE id = 1
        `).get();
        ...
        if (!admin.admin_email) {
            db.prepare(`
                ...
                WHERE id = 1
            `).run(cleanEmail);
        } else if (admin.admin_email !== cleanEmail) {
            return res.status(401).json({
                success: false,
                message: "That Gmail address is not registered for the admin account."
            });
        }
        ...
        await transporter.sendMail({
            ...
            to: cleanEmail,
            subject: "UniVote Admin Password Reset Code",
            text: `Your UniVote admin password reset code is ${code}.
...
async function resetAdminPassword(req, res) {
    try {
        ...
        if (!admin.admin_email || admin.admin_email !== cleanEmail) {
            return res.status(401).json({
                success: false,
        ...
        if (Number(admin.reset_attempts || 0) >= 5) {
        ...
        if (!admin.reset_code_hash || !admin.reset_expires_at) {
            return res.status(400).json({
                success: false,
        ...
        if (new Date(admin.reset_expires_at).getTime() < Date.now()) {
            db.prepare(`
                UPDATE admin_settings
                SET
        ...
        if (submittedHash !== admin.reset_code_hash) {
            db.prepare(`
                UPDATE admin_settings
...
function createElection(req, res) {
    try {
        ...
        if (!name || !startDate || !endDate) {
            return res.status(400).json({
                success: false,
                message: "Election name, start date and end date are required."
            });
        }
        ...
        if (existing) {
            return res.status(409).json({
                success: false,
                message: "An election with this name already exists."
            });
        }
        ...
        console.error("CREATE ELECTION DATABASE ERROR:", error);
...
function deleteElection(req, res) {
    try {
        ...
        return res.status(500).json({
            success: false,
            message: "Unable to delete election."
        });
    }
}
